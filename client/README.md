...
The most challenging part was the authentication middleware. Getting JWT and API key authentication to coexist in the same middleware, with the correct Elysia plugin scoping, required careful debugging. The key insight was that `.as("scoped")` is required for middleware to propagate correctly through Elysia's plugin system.

The most satisfying part was the payout calculation  writing an algorithm that correctly distributes a prize pool proportionally, handles the edge case of no winners, and wraps everything in a transaction that guarantees consistency regardless of server state.
